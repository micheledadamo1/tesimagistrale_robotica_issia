...
class full_nl_controller_class():
    ...
    def output_controller(self):
        """
        metodo utilizzato per la simulazione in gazebo
        u -> normalizzata in 0 - 1
        """
        u=norm(self.c_pos.F_tot)*(0.513)/8.91 #si necessitano esperimenti per capire la "normalizzazione" del thrust

        torques=self.c_att.tau
